...
class Shopsell(QWidget):
    ...
    def event_lr(self):
        if self.line_txm.text != '':
            try:
                ...
                txm = 0
                # replay = QMessageBox.warning(self, "!", "请输入正确的条形码!", QMessageBox.Yes)
        else:
            txm = 0
            replay = QMessageBox.warning(self, "!", "请输入正确的条形码!", QMessageBox.Yes)
        if self.line_xssl.text() != '':
            ...
            xssl = -1
            replay = QMessageBox.warning(self, "!", "请输入正确的商品数量!", QMessageBox.Yes)
        ...
        for i in range(len(row)):
            r.append(row[i][0])
        if txm not in r:
            replay = QMessageBox.warning(self, "!", "商品不存在!", QMwssageBox.Yes)
        else:
            #将欲销售的商品数量与库存量相比较, 如果小于就报出提醒
            cur.execute("select kcl from Inventory where txm=%d" % txm) 
            row = cur.fetchone()
            if xssl > row[0] or xssl == -1:
                replay = QMessageBox.warning(self, "!", "商品库存不足!", QMessageBox.Yes)
            else:
                try:
                    cur.execute("select * from Inventory where txm=%d" % txm)
                    rows = cur.fetchone()
                    #print(rows)
                    newItem1 = QTableWidgetItem(str(rows[0]))
                    newItem2 = QTableWidgetItem(str(rows[1]))
                    newItem3 = QTableWidgetItem(str(rows[4]))
                    newItem4 = QTableWidgetItem(str(xssl))
                    newItem5 = QTableWidgetItem(str(float("%0.1f"%(rows[4]*xssl))))
                    ...
                    if self.line_sell1.text != '':
                        self.line_sell1.setText(str(float("%0.1f"%(float(self.line_sell1.text())+rows[4]*xssl))))
                    else:
                        self.line_sell1.setText(str(float("%0.1f"%(rows[4]*xssl))))

                    self.line_sell2.setText(str(self.line_sell1.text()))
    ...
    def jiesuan(self):
        try:
            self.line_sell4.setText("%.1f" % (float(self.line_sell3.text()) - float(self.line_sell2.text())))
    ...
    def event_qr(self):
        ...
        if self.line_sell1.text == "0.0":
            ...
            for i in range(l):
                ...
                cur.execute(("insert into Sellgoods values (%s, %s, %s, %s)"), (self.txm[i], self.lsj[i], self.xssl[i], self.xssj[i]))
                cur.execute("select kcl from Inventory where txm=%d"%self.txm[i])
                row = cur.fetchone()
                cur.execute("update Inventory set kcl=%d where txm=%d"%(row[0]-self.xssl[i], self.txm[i]))
            conn.commit()
            conn.close()
