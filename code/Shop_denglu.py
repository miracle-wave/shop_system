...
class Denglu(QWidget):
    ...
    创建完成时把账号及密码存储在文件中：账号.txt、密码.txt  并退出关闭创建窗口
    ...
    def check(self):
        ...
        for line1 in id:
            ...
            for line2 in password:
                ...
                if i < m:
                    continue
                #如果输入账号不在账号表文件中, 则推送消息提醒
        if self.lineedit_id.text() + "\n" not in self.id_password:
            replay = QMessageBox.warning(self, "!", "账号密码输入错误", QMessageBox.Yes)
        else:
    ...
    def idnewqueren(self):
        ...
        if self.lineedit_idnew_id.text() == "":
            replay = QMessageBox.warning(self, "!", "账号不能为空", QMessageBox.Yes)
        else:
            if self.lineedit_idnew_id.text() + "\n" in self.id_password:
